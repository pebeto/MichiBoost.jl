function _fill_num_hist!(buf, li, group, j, gradients, hessians, num_bins, nb)
    nb1 = nb + 1
    ...
    if n >= 512
        ...
        @inbounds for i in 1:n
            ...
            buf.hist_g[li, b] += local_g[i]
            buf.hist_h[li, b] += local_h[i]
            buf.hist_c[li, b] += 1
            g_sum += local_g[i]
            h_sum += local_h[i]
        end
        ...
        @inbounds for idx in group
            ...
            buf.hist_g[li, b] += gradients[idx]
            buf.hist_h[li, b] += hessians[idx]
            buf.hist_c[li, b] += 1
            g_sum += gradients[idx]
            h_sum += hessians[idx]
        end
    end
    buf.total_g[li] = g_sum
    buf.total_h[li] = h_sum
    buf.total_n[li] = n
end

function _fill_cat_hist!(buf, li, group, j, gradients, hessians, cat_encoded, sorted_vals, nv)
...
function _sweep_gain(buf, n_leaves, nb_or_nv, l2_leaf_reg, min_data_in_leaf)
    best_gain = -Inf
    best_b = -1
    @inbounds for li in 1:n_leaves
        buf.left_g[li] = buf.hist_g[li, 1]
        buf.left_h[li] = buf.hist_h[li, 1]
        buf.left_c[li] = buf.hist_c[li, 1]
    end
    @inbounds for b in 2:nb_or_nv
        @inbounds for li in 1:n_leaves
            buf.left_g[li] += buf.hist_g[li, b]
            buf.left_h[li] += buf.hist_h[li, b]
        ...
        @inbounds for li in 1:n_leaves
            ...
            rg = buf.total_g[li] - buf.left_g[li]
            rh = buf.total_h[li] - buf.left_h[li]
            gain +=
                buf.left_g[li]^2 / (buf.left_h[li] + l2_leaf_reg) +
                rg^2 / (rh + l2_leaf_reg) -
                buf.total_g[li]^2 / (buf.total_h[li] + l2_leaf_reg)
        end
        if gain > best_gain
            best_gain = gain
            best_b = b
...
function _find_best_split_across_leaves(
    ...
    Threads.@threads :static for j in sampled_num
        ...
        for (li, group) in enumerate(leaf_groups)
            _fill_num_hist!(buf, li, group, j, gradients, hessians, num_bins, nb)
        end
        gain, b = _sweep_gain(buf, n_leaves, nb + 1, l2_leaf_reg, min_data_in_leaf)
    ...
    Threads.@threads :static for j in sampled_cat
        ...
        buf = bufs[tid]
        for (li, group) in enumerate(leaf_groups)
            _fill_cat_hist!(buf, li, group, j, gradients, hessians, cat_encoded, sorted_vals, nv)
        end
        gain, b = _sweep_gain(buf, n_leaves, nv, l2_leaf_reg, min_data_in_leaf)
