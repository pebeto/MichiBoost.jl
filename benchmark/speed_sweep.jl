...
const GRID =
    QUICK ? [(500, 10), (5_000, 20)] : [(500, 10), (2_000, 20), (10_000, 50), (50_000, 50)]
...
if SECTION in ("all", "categorical")
    ...
    for k in CAT_CARDINALITIES
        ...
        _, jl_df_tr, cb_pool_tr, jl_pool_tr, _ = make_cat_frames(
            X_num[tr, :], X_cat[tr, :], y[tr]
        )
        ...
        tag = su >= 1.0 ? "MB $(round(su; digits=2))x" : "CB $(round(1/su; digits=2))x"
        println(
            "  cardinality=$(lpad(k, 4))  CatBoost $(rpad(round(t_cb; digits=1), 8))ms  MichiBoost $(rpad(round(t_mb; digits=1), 8))ms  $tag",
        )
    end
end
...
if SECTION in ("all", "inference")
    ...
    for b in BATCH_SIZES
        ...
        tag = su >= 1.0 ? "MB $(round(su; digits=2))x" : "CB $(round(1/su; digits=2))x"
        println(
            "  batch=$(lpad(b, 6))  CatBoost $(rpad(round(t_cb; digits=3), 9))ms  MichiBoost $(rpad(round(t_mb; digits=3), 9))ms  $tag",
        )
    end
end
