...
function cmp_row(name, t_mb, t_cb=nothing)
    if t_cb === nothing
        ...
        println(
            "  $(rpad(name, 22))  CatBoost $(rpad(round(t_cb; digits=1), 8))ms  MichiBoost $(rpad(round(t_mb; digits=1), 8))ms  $tag",
        )
    end
end

# Fixtures (single moderate-size dataset keeps comparisons apples-to-apples)
X_reg_tr, y_reg_tr, X_reg_te, y_reg_te = train_test_split(
    regression_data(; n=2000, p=20)...
)
...
let
    ...
    t_full =
        median(
            @benchmark(MichiBoost.fit!($(mk(1.0))(), $jl_pool), samples = 3, evals = 1)
        ).time / 1e6
    t_half =
        median(
            @benchmark(MichiBoost.fit!($(mk(0.5))(), $jl_pool), samples = 3, evals = 1)
        ).time / 1e6
...
let
    ...
    t_uw =
        median(@benchmark(MichiBoost.fit!($mk(), $pool_uw), samples = 3, evals = 1)).time /
        1e6
    t_w =
        median(@benchmark(MichiBoost.fit!($mk(), $pool_w), samples = 3, evals = 1)).time /
        1e6
